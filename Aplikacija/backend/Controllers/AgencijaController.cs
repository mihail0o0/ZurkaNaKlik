...
namespace backend.Controllers
{
    ...
    public class AgencijaController : ControllerBase
    {
        ...
        public  async Task<IActionResult> AzurirajKategoriju([FromBody]Kategorija kategorija)
        {
            try {
                int idAgencije = int.Parse((HttpContext.Items["idAgencije"] as string)!);
        ...
        #region VratiKategoriju

        [HttpPut("VratiKategoriju/{idkategorije}")]
        public  async Task<IActionResult> VratiKategoriju(int idkategorije)
        {
            try {
                var kategorija = await Context.Kategorije.FindAsync(idkategorije);
